...
def simulate_null(X):
    ...
    return K.first_test(X.astype(np.float), Z, sigma=np.sqrt(sigma**2+9))
...
def fig2(nsim=10000):
    ...
    X -= X.mean(0)
    X /= X.std(0)
    fig(X, 'fat_lasso_texp.pdf', nsim=nsim)
...
def fig3(nsim=10000):
    ...
    X -= X.mean(0)
    X /= X.std(0)
    fig(X, 'tall_lasso_texp.pdf', nsim=nsim)
...
''')
    X = IP.user_ns['X']
    fig(X, 'lars_diabetes_texp.pdf', nsim=nsim)
