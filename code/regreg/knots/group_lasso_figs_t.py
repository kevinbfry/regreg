...
def fig2(nsim=10000):
    ...
    fig(X, 'fat_group_lasso_t.pdf', groups, nsim=nsim)
...
def fig3(nsim=10000):
    ...
    fig(X, 'tall_group_lasso_t.pdf', groups, nsim=nsim)
...
def fig4(nsim=10000):
    ...
    X /= X.std(0)
    groups = np.array(range(10)*10)
    fig(X, 'square_group_lasso_t.pdf', groups, nsim=nsim)
...
''')
    ...
    fig(X, 'lars_diabetes_group_t.pdf', groups, nsim=nsim, weights={2:1,3:2,0:4})
...
''')
    ...
    fig(X, 'lars_diabetes_lasso_as_group_t.pdf', groups, nsim=nsim, 
...
def fig7(nsim=10000):
    ...
    fig(X, 'nested_groups_big_first_t.pdf', groups, nsim=nsim, weights={0:0.1,1:3})
...
def fig8(nsim=10000):
    ...
    X[:,8:] = X[:,:2]
    groups = np.array([0]*8+[1]*2)
    fig(X, 'nested_groups_smaller_first_t.pdf', groups, nsim=nsim, weights={1:0.1,0:3})
...
def fig9(nsim=10000):
    ...
    fig(X, 'two_nested_groups_t.pdf', groups, nsim=nsim)
...
def fig10(nsim=10000):
    ...
    X -= X.mean(0)
    X /= X.std(0)
    fig(X, 'several_nested_groups_t.pdf', groups, nsim=nsim, weights=
        {0:1.9,2:1.9,5:1.3})
