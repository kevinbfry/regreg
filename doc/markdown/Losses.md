...
- Cox proportional hazards (`regreg.glm.glm.coxph`, depends on `statsmodels`)
...
```
loss.solve()
%R -i X,Y print(coef(glm(Y ~ X-1, family=binomial())))
print(loss.solve())
```
...
coef(G, s=2 / nrow(X), exact=TRUE)
```

Suppose we want to match `glmnet` exactly without having to 
specify `intercept=FALSE` and `standardize=FALSE`.
The `normalize` transformation can be used here.
...
%%R -o coefG
library(glmnet)
G = glmnet(X, as.numeric(Y), standardize=FALSE, intercept=TRUE, family='binomial')
...
```
coefG = np.asarray(coefG)
problem_normalized.objective(coefG), problem_normalized.objective(coefR)
```

In theory, using the `standardize=TRUE` option in `glmnet` should be the same as using `scale=True, value=np.sqrt((n-1)/n)` in `normalize`, though
the results don't match without some adjustment. This is because
`glmnet` returns coefficients that are on the scale of the original 
$X$. 
...
```
coefG = np.asarray(coefG) * X_normalized.col_stds
problem_normalized.objective(coefG), problem_normalized.objective(coefR)
```

## Defining a new smooth function

A smooth function only really needs a `smooth_objective` method 
in order to be used in `regreg`. 
...
$$
where $I^{\infty}_K$ is the indicator of $K=\left\{\mu: a_i^T\mu\leq b_i, 1 \leq i \leq k\right\}$ (i.e. 0 inside $K$ and $\infty$ outside $K$).
...
The Huber loss is built into regreg, but can also be obtained
by smoothing the `l1norm` atom. We will verify the two
...
of an `atom` (`positive_part`) with an affine transform
determined by the data.

Such objective functions can be smoothed. [NESTA](TODO) and [TFOCS](TODO)
describe schemes in which smoothing of these atoms can be used to
produce optimization problems with smooth objectives which can have
additional structure imposed through optimization.

Let us try smoothing the objective and using NESTA by smoothing the 
hinge loss. Of course, one can also solve the usual SVC dual problem by smoothing.
...
def nesta_svm(X, y_pm, C=1.):
    ...
    smooth_ = rr.quadratic.linear(selector)
...
Using `regreg`, we can easily add penalty or constraint to the
SVM objective.
...
Instead of using NESTA we can just smooth the SVM with a fixed
smoothing parameter and solve the problem directly.
...
huber_svm = huberized_svm(X_l_inter, Y_l, smoothing_parameter=0.001, C=C)
